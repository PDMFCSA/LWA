...
} from "../utils/leafletUtils.js";
import {
  translate,
  getTranslation,
  transformToISOStandardLangCode,
  getLanguageFallback,
...
const sanitationRegex =
  /(<iframe>([\s\S]*)<\/iframe>)|(<script>([\s\S]*)<\/script>)/g;
...
function LeafletController() {
  ...
  this.lsEpiDomain =
    (environment.enableEpiDomain
      ? localStorage.getItem(constants.EPI_DOMAIN)
      : environment.epiDomain) || environment.epiDomain;
  ...
  this.showModal = function (modalId, stackModal = false) {
    ...
    if (!stackModal) {
      ...
      if (activeModal) activeModal.classList.add("hiddenElement");
    }
  ...
  const getLeafletMetadata = () => {
    ...
    this.leafletService
      .getLeafletMetadata(
        this.timePerCall,
        this.totalWaitTime,
        this.gto_TimePerCall,
        this.gto_TotalWaitTime,
      )
      .then((data) => {
        ...
        if (!tmp || sanitationRegex.test(tmp))
          return goToErrorPage(
            constants.errorCodes.unsupported_response,
            new Error(
              "Response unsupported format or contains forbidden content",
            ),
          );

        this.metadata = data;
        setTimeout(() => {
          showRecalledMessage(data);
        }, 100);

        if (
          typeof data.availableDocuments === "string" &&
          data.availableDocuments === "xml_found"
        ) {
  ...
  const getLeafletXML = () => {
    ...
    this.leafletService.leafletType =
      this.selectedDocument === DocumentsTypes.INFO
        ? DocumentsTypes.PRESCRIBING_INFO
        : this.selectedDocument;

    this.leafletService
      .getLeafletUsingCache(
        this.timePerCall,
        this.totalWaitTime,
        this.gto_TimePerCall,
        this.gto_TotalWaitTime,
  ...
  this.closeModal = function (evt) {
    const modalId =
      typeof evt === "string"
        ? evt
        : evt.currentTarget.getAttribute("modal-id");
    if (
      ["leaflet-lang-select", "documents-modal", "epi-markets-modal"].includes(
        modalId,
      )
    )
  ...
  const showAvailableMarkets = (availableMarkets) => {
    ...
    availableMarkets
      .map((item) => {
        const name =
          item !== "unspecified" ? getCountry(item, true) : "unspecified";
        return { item, name };
      })
      .sort((a, b) => {
        if (a.item === "unspecified") return a;
        if (b.item === "unspecified") return b.item;
        return a.name.localeCompare(b.name, this.defaultLanguage, {
          sensitivity: "base",
        });
      })
      .forEach((pair, index) => {
        ...
        const label =
          item !== "unspecified"
            ? name
            : getTranslation("epi_markets_modal_no_market");
        ...
        radioFragment.setAttribute(
          "aria-checked",
          new Boolean(index === 0).toString(),
        );
    ...
    modal
      .querySelector("#epi-market-go-back-button")
      .addEventListener("click", () => this.goHome());

    modal
      .querySelector("#epi-market-proceed-button")
      .addEventListener("click", () => {
        const value = modal.querySelector(
          "input[name='epi-market']:checked",
        )?.value;
  ...
  this.setSelectEpiMarket = function (selectedMarket) {
    ...
    const availableLanguages =
      this.metadata.availableDocuments[this.selectedDocument][
        this.selectedEpiMarket
      ];
  ...
  const showDocumentModal = (result) => {
    try {
      ...
      console.error(e);
      goToErrorPage(
        constants.errorCodes.xml_parse_error,
        new Error("Unsupported format for XML file."),
      );
    }
  };
  ...
  const showAvailableDocuments = (result) => {
    let documents = [
      ...
      {
        text: "document_prescribing_info",
        value: DocumentsTypes.PRESCRIBING_INFO,
      },
    ];
    ...
    const hasPrescribingInfo = productDocuments.includes(
      DocumentsTypes.PRESCRIBING_INFO,
    );

    if (!hasLeaflet)
      documents = documents.filter(
        (doc) => doc.value !== DocumentsTypes.LEAFLET,
      );

    if (!hasPrescribingInfo)
      documents = documents.filter(
        (doc) => doc.value !== DocumentsTypes.PRESCRIBING_INFO,
      );

    const { markets } = result?.productData || {};
    this.productMarkets = markets || [];
    if (
      !markets ||
      markets.length < 1 ||
      !markets.some((market) =>
        constants.MARKETS_WITH_PRODUCT_INFORMATION.includes(market.marketId),
      )
    ) {
    ...
    if (!documents?.length)
      return goToErrorPage(
        constants.errorCodes.no_uploaded_epi,
        new Error(`Has not documents for product`),
      );
    ...
    const container = modal.querySelector("#content-container");
    const radioParent =
      container.querySelector("#documents-radio-container") ||
    ...
    documents.forEach((item, index) => {
      ...
      if (index === 0) selectedItem = radioFragment;

      radioFragment
        .querySelector("input")
        .addEventListener("change", (event) => {
          if (selectedItem) selectedItem.setAttribute("aria-checked", "false");
          radioFragment.setAttribute("aria-checked", "true");
          selectedItem = radioFragment;
        });
  ...
  this.setSelectedDocument = function (selectedDocument = null) {
    this.selectedDocument = selectedDocument
      ? selectedDocument
      : document.querySelector("input[name='documents']:checked")?.value;
    if (this.selectedDocument === DocumentsTypes.INFO) {
      this.selectedEpiMarket = "";
    ...
    for (const [key, value] of Object.entries(
      ...
      if (value && value?.length) marketsWithResult.push(key);
    }
  ...
  this.getLanguageFromBrowser = function (fallback = true) {
    ...
    browserLang =
      getLanguageFallback(browserLang.toLowerCase(), fallback) || browserLang;
  ...
  const showAvailableLanguages = (languages) => {
    ...
    if (languages.length >= 1) {
      ...
      if(Number(this.scantype) === 1 && this.productMarkets.some((market) => market.marketId === "TR")) {
        if(languages.length === 1)
          return setSelectedLanguage(languages[0].value.toLowerCase());        
      }

      const modal = this.showModal("leaflet-lang-select");
      if (this.selectedDocument === DocumentsTypes.INFO) {
        modal.querySelector("#language-message").textContent = getTranslation(
          "document_lang_select_message",
        );
        modal.querySelector("#lang-title").textContent = getTranslation(
          "document_lang_select_title",
        );
      }
      ...
      modal
        .querySelector(".proceed-button.no-leaflet")
      ...
      languages.forEach((lang, index) => {
        ...
        labelDiv.textContent = escapeHTML(
      ...
      goToErrorPage(
        constants.errorCodes.no_uploaded_epi,
        new Error(`Product found but no associated leaflet`),
      );
  ...
  const changeAvailableLanguages = (languages) => {
    ...
    modal.querySelector("#change-language-message").textContent =
      getTranslation("document_change_lang_message");
    modal.querySelector("#change-lang-title").textContent = getTranslation(
      "document_change_lang_title",
    );
    ...
    modal.querySelector("#proceed-button").addEventListener("click", () => {
      ...
      if (lang !== this.selectedLanguage) setSelectedLanguage(lang);
  ...
  let showRecalledMessage = (result) => {
    ...
    if (recalled) {
      ...
      const recalledMessageContainer = document.querySelector(
        ".recalled-message-container",
      );
      ...
      if (batchRecalled) {
        recalledContainer.querySelector("#recalled-title").textContent =
          getTranslation("recalled_batch_title");
        recalledMessageContainer.innerHTML = getTranslation(
          "recalled_batch_message",
          `<strong>${batchData?.batch || batchData.batchNumber}</strong>`,
        );
        recalledBar.querySelector("#recalled-bar-content").textContent =
          getTranslation("leaflet_recalled_batch");
        recalledMessageContainer.innerHTML +=
          "<br /><br />" +
          getTranslation(
            "recalled_product_name",
            `<strong>${result.productData.nameMedicinalProduct}</strong>`,
          );
      } else {
        recalledMessageContainer.innerHTML += getTranslation(
          "recalled_product_message",
          `<strong>${result.productData.nameMedicinalProduct}</strong>`,
        );
      }
      ...
      recalledContainer.querySelector("#recalled-modal-procced").onclick =
        function () {
          recalledContainer.classList.add("hiddenElement");
          activeModal.classList.remove("recalled");
        };
      recalledContainer.querySelector("#recalled-modal-exit").onclick =
        function () {
          goToPage("/main.html");
        };
  ...
  this.showPrintVersion = (modal = "settings-modal") => {
    ...
    window.onbeforeprint = (evt) => {
      if (!evt.target.document) evt.target.document = { title: "" };
          ...
          if (replaced !== node.textContent) {
            ...
            if (element) {
              const section = element.closest(
                ".section.leaflet-accordion-item",
              );
  ...
  const addEventListeners = () => {
    document
      .getElementById("search-modal-button")
    ...
    document
      ...
      .addEventListener("click", this.scanAgainHandler);
    document
      .getElementById("modal-print-button")
      .addEventListener("click", this.printContent.bind(this));
    document
      .querySelectorAll("#print-modal-button")
      .forEach((button) =>
        button.addEventListener("click", this.showPrintModal),
      );
    document
      .querySelectorAll("#languages-modal-button")
      .forEach((button) =>
        button.addEventListener("click", this.LoadAvailableLanguages),
      );
    document
      .getElementById("modal-scan-again-button")
      .addEventListener("click", this.scanAgainHandler);
    document
      .querySelectorAll("#go-back-button")
      .forEach((button) => button.addEventListener("click", this.goHome));
    document
      .getElementById("modal-print-go-back-button")
      .addEventListener("click", this.closePrintModal);
    document
      .querySelectorAll(".modal-container.popup-modal .close-modal")
      .forEach((item) => {
        item.addEventListener("click", this.closeModal);
      });
    document
      .querySelector("#documents-modal #proceed-button")
      .addEventListener("click", () => {
        this.setSelectedDocument();
      });
    document
      .querySelector("#product-modal #button-exit")
      .addEventListener("click", () => {
        goToPage("/scan.html");
      });
  };
